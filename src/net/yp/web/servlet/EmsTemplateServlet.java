...
public class EmsTemplateServlet extends HttpServlet {
	...
	private EmsTemplateService emsTemplateService = (EmsTemplateService) Context
			.getApplicationContext().getBean("emsTemplateService");

	private CommodityService commodityService = (CommodityService) Context
			.getApplicationContext().getBean("commodityService");

	private static Logger logger = Logger
			.getLogger(EmsTemplateServiceImpl.class);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if ("addQa".equals(type)) {// 新增问答模板
				...
				// JSONSerializer.toJSON(msg);
				// Map<String, Object> map = new HashMap<String, Object>();
				// map.put("questions", QuestionAndAnswer.class);
				// QaTemplate qaTemplate =
				// (QaTemplate)JSONObject.toBean(jsonObject,QaTemplate.class,map);
				// InputStream is = new
				// ByteArrayInputStream(qaTemplate.getPictureSource().getBytes());
				// BufferedImage image = ImageIO.read(is);
				// String filePath =
				// EmsUtil.getPropertiesForName("imagePath")+qaTemplate.getPictureName();
				// File outputFile = new File(filePath);
				// ImageIO.write(image, qaTmplate.getPictureType(),
				// outputFile);//将文件写入磁盘
				// qaTemplate.setPictureUrl(filePath);
				...
				for (int i = 0; i < fileItems.size(); i++) {
					...
					if (item.isFormField()) {
						...
						if ("name".equals(name)) {
							...
							commodity.setCostPrice(Float.parseFloat(value));
						} else if ("currentPrice".equals(name)) {
							commodity.setCurrentPrice(Float.parseFloat(value));
